...
export default class StockService extends LeekService {
  ...
  isPanqianPanhou(): boolean {
    ...
    // 计算夏令时开始和结束日期
    // 夏令时：3月第二个周日开始，11月第一个周日结束
    const dstStart = this.getNthSundayOfMonth(year, 3, 2); // 3月第二个周日
    const dstEnd = this.getNthSundayOfMonth(year, 11, 1);   // 11月第一个周日

    // 判断当前是否为夏令时
    const isDST = now >= dstStart && now < dstEnd;

    // 根据夏令时确定时区偏移（UTC-4夏令时，UTC-5冬令时）
    const timezoneOffset = isDST ? -4 : -5;

    // 将当前时间转换为美国东部时间
    const etTime = new Date(now.getTime() + (timezoneOffset * 60 * 60 * 1000) + (now.getTimezoneOffset() * 60 * 1000));
    ...
    // 盘前：04:00-09:30
    // 盘后：16:00-20:00
    const isPreMarket = etTimeInMinutes >= preMarketStart && etTimeInMinutes < marketOpen;
    const isAfterMarket = etTimeInMinutes >= marketClose && etTimeInMinutes < afterMarketEnd;

    return isPreMarket || isAfterMarket;
  }

  // 辅助函数：获取指定月份的第N个周日
  private getNthSundayOfMonth(year: number, month: number, nth: number): Date {
